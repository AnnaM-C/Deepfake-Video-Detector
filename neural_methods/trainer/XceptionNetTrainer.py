...
from sklearn.metrics import confusion_matrix, ConfusionMatrixDisplay
import matplotlib.pyplot as plt
from neural_methods.model.XceptionNet import Xception

class XceptionNetTrainer(BaseTrainer):
    ...
    def __init__(self, config, data_loader):
        ...
        self.model = Xception(xception_config).to(self.device)

        if config.TOOLBOX_MODE == "train_and_test":
            self.num_train_batches = len(data_loader["train"])

            if len(config.TRAIN.DATA.DATASET.replace(" ", "").split(",")) == 3:
                weights = torch.tensor([3.0, 1.0], dtype=torch.float32)
                if torch.cuda.is_available():
                    weights = weights.cuda()
                self.loss_model = nn.CrossEntropyLoss(weight=weights)
                print("Update weight balancing!")
            else:
                self.loss_model = nn.CrossEntropyLoss()
            ...
            # state_dict = torch.load("pretrained/Xception/xception-b5690688.pth")
